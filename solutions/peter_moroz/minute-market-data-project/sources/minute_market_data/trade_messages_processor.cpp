...
mcast_comm::trade_messages_processor::trade_messages_processor(bool skip_chain_init)
  : next_(NULL)
{
  if (!skip_chain_init)
    init_handlers_chain();
...
void mcast_comm::trade_messages_processor::init_handlers_chain()
{
    ...
    const std::string& m)
{
  ...
  trade_message_ptr tmessage_ptr(new trade_message(ts, ss, p, v));
  return tmessage_ptr;
}
