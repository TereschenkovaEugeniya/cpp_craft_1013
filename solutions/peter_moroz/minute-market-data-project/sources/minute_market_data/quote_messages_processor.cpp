...
const size_t kTimeStampOffset = 18;
const size_t kTimeStampLength = 6;
...
mcast_comm::quote_messages_processor::quote_messages_processor(bool skip_chain_init) 
  : next_(NULL)
{
  if (!skip_chain_init)
    init_handlers_chain();
...
void mcast_comm::quote_messages_processor::init_handlers_chain()
{
...
mcast_comm::quote_message_ptr 
    ...
    const std::string& m)
{
  std::string ts = m.substr(kTimeStampOffset, kTimeStampLength);
	size_t offset = kMessageHeader;

	std::string ss = m.substr(offset, kSecuritySymbol);
  ...
  quote_message_ptr qmessage_ptr(new quote_message(ts, ss,
                                 bid_price, bid_volume, 
                                 offer_price, offer_volume));
  return qmessage_ptr;
}
