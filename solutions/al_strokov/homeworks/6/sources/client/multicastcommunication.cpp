...
multicast_communication::multicast_communication(size_t tradeThreadsCount,
	...
	for (size_t i = 0; i < quotesEndpoints_.size(); i++) {
		...
		quoteSockets_.push_back(new boost::asio::ip::udp::socket(quoteService_));
	...
	for (size_t i = 0; i < tradesEndpoints_.size(); i++) {
		...
		tradeSockets_.push_back(new boost::asio::ip::udp::socket(tradeService_));
		char *b = new char[buffMaxSize_];
...
multicast_communication::~multicast_communication() {
	std::cout << "multicastcomm clean up" << std::endl;
...
void multicast_communication::registerQuoteListeners() {
	for (size_t i = 0; i < quotesEndpoints_.size(); i++) {
		quoteSockets_[i]->async_receive(boost::asio::buffer(quoteBuffs_[i], buffMaxSize_),
...
void multicast_communication::registerTradeListeners() {
	for (size_t i = 0; i < tradesEndpoints_.size(); i++) {
		tradeSockets_[i]->async_receive(boost::asio::buffer(tradeBuffs_[i], buffMaxSize_),
...
void multicast_communication::kill() {
	std::cout << "killing communicator" << std::endl;
...
void multicast_communication::run() {
	...
	for (size_t i = 0; i < quoteThreadsCount_; i++) {
		quotesPool.create_thread(
				boost::bind(&multicast_communication::startQuoteServices, this));
	}

	boost::thread_group tradesPool;
	for (size_t i = 0; i < tradeThreadsCount_; i++) {
		tradesPool.create_thread(
				boost::bind(&multicast_communication::startTradeServices, this));
	}

	std::cout << "multicast_communication is running...\nPress CTRL+C to shutdown\n;";
	quotesPool.join_all();
	tradesPool.join_all();
}
...
void multicast_communication::extractTradeMessage(
	...
	tradeMessage.price = atof(
			chars2string(shortTrade.tradePrice, 8).c_str());
	tradeMessage.volume = atof(
			chars2string(shortTrade.tradeVolume, 4).c_str());
}
...
void multicast_communication::extractTradeMessage(const longTrade_t& longTrade,
	...
	tradeMessage.security_symbol = strSecuritySymbol;
	tradeMessage.price = atof(
			chars2string(longTrade.tradePrice, 12).c_str());
	tradeMessage.volume = atof(
			chars2string(longTrade.tradeVolume, 9).c_str());
}
